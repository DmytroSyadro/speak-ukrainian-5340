...
test.describe('news-navigation', (): void => {
  ...
  test('tc-56: Verify pagination on the News page', async (): Promise<void> => {
    ...
    await allure.step('Step 1: Scroll down to the pagination component at the bottom of the news list', async () => {
  ...
  test('tc-58: Verify that changing the global city redirects to clubs, and returning to news displays the updated sidebar', async ({ page }): Promise<void> => {
    ...
    await allure.step('Step 4: Change the global city selector in the header to a different city', async () => {
        await newsPage.header.selectCity(CitiesUser.KHARKIV);
    });

    await allure.step('Step 5: Verify that the Clubs page is opened for the selected city', async () => {
        await page.waitForURL('**/clubs*');
        expect(page.url()).toContain('/clubs');
        await newsPage.header.hasCitySelected(CitiesUser.KHARKIV);
    });

    await allure.step('Step 6: Click on the "Новини" link inside the main header to return to the news list', async () => {
        await newsPage.header.clickNews();
        await page.waitForURL('**/news*');
        await newsPage.waitForPageLoad();
    });

    await allure.step('Step 7: Verify the title of the sidebar after returning to the News page', async () => {
        expect(await sidebar.getSidebarTitle()).toBe('Гуртки у місті Харків');
    });

    await allure.step('Step 8: Verify the location/address text on the first club card in the sidebar', async () => {
        const firstCard = await sidebar.getClubCardByIndex(0);
        const address = await firstCard.getClubAddress();
        expect(address).toContain('Харків');
    });

    await allure.step('Step 9: Click on the "Детальніше" button of the first club in the sidebar', async () => {
        const firstCard = await sidebar.getClubCardByIndex(0);
  ...
  test('tc-59: Verify the full user journey of browsing, reading, and navigating between multiple news articles', async ({ page }): Promise<void> => {
    await allure.epic('Speak Ukrainian');
    await allure.owner('Petro Derlytsia');
    await allure.feature('News page');
    ...
    await allure.step('Step 2: Remember the title of the first news card on page 2, and click its "Детальніше ->" link', async () => {
    ...
    await allure.step('Step 4: Scroll down to the "Інші новини" (Other news) carousel section at the bottom', async () => {
    ...
    await allure.step('Step 5: Remember the title of the first visible card in the carousel and click on it', async () => {
    ...
    await allure.step('Step 7: Click on the "Новини" link inside the main website header', async () => {
        await newsPage.header.clickNews();
    });
